...
class DeepTMHMM:
    ...
    def run(
        self,
        seqs: abc.Iterable[ChainSequence] | abc.Iterable[tuple[str, str]] | Path,
        blocking: bool = True,
        parse: bool = True,
    ):
                    ...
                    if j.is_finished():
                        try:
                            outputs.append(
                                (
                                    i,
                                    self.parse_output_gff(
                                        j.get_output_file("/TMRs.gff3").get_data()
                                    ),
                                )
                            )
                            ...
                            LOGGER.exception(e)
                            LOGGER.warning(
                                f"Failed to complete a job {j.id} due to {e}"
                            )
                        jobs.remove(x)
                        bar.update(1)
                if jobs:
